...
проекта во время разработки менялся (захотелось добавить комментарии,
 ...
 структуре данных и структур их отображения, в следствии этого рефакторинг и реорганизация кода 
 ...
 Так вот при перетаскивании элементов малых размеров, они смещались за пределы курсора (в заисимости от его положения в контейнере комнаты) 
 и событие pointerUp не срабатывало, тк находился обработчик события на элементе и 
 "drag and drop" не завершался. Решил эту проблему использванием цепочки условий, где в зависимости от 
 местоположения курсора отнималась/прибавлялась разница этого смещения в позиционирование элемента.`

 export const TheFurniture_further = `В планах на будущее собираюсь сделать мобильную версию этого веб-приложения. 
  Компоненты, в которых большие простыни кода, разбить на более мелкие компоненты, это улучшит не только читаемость
   , но и переиспользуемость кода. 
  Добавить сортировку товара по различным критериям (размещать товары по возростанию цены, рейтингу и др),
  возможность изменения оценки товара для пользоваателя, .
  ...
  которые присутствуют на проекте (ползунок рейтинга в фильтре, более интересный UI пагинации).
   `
export const GtaHud_problem = `В принципе проект не сложный, затруднения были только в работе с обьектом EventMeneger и векторной графикой
 (про svg теги до этого только читал). EventMeneger — это обект, в котором храняться события, у него есть парочка методов для работы с этими 
 событиями (создание, удаление и изменение), надо сказать, что пользовался только созданием и удалением. Как работать с этим обьектом
  ...
  формы у них были не стандартные ( 3/4 круга, анимация пульса сердца и др), поэтому единственным оптимальным решением было
   использование svg тегов (circle, path, linearGradient и других). Практически все компоненты независимы и могут быть переиспользованы,
   например, индикаторы воды, еды и топлива индентичны, меняется только иконка (капля, бензоколонка и вилка с ножом), я написал один компонент 
   для них, через *пропсы передавал ссылку на иконку и колличество единиц, которые остались у игрока. Подобным образом создавал уведомления,
...
Рабочий *процес был построен таким образом, что к концу недели дизайнер доделывал какой-то 
макет (например: интерфейс при смерти и реанимации) и на следующей недели я по ниму писал UI код.
 Созванивались с PM в конце надели, рассказывал и покозвал, что сделал за этот спринт. Два раза
...
`
export const Lending_problem = `Проблемы которые возникали в процессе работы над этим преоктом,
...
переиспользован и это в добавок повысело бы читаемость кода. Использовал бы комбинаторные и переиспользуемые
...
к сторонему ресурсу. Пользователь может в поле ввода ввести город, в котором он хочет узнать погоду, и получит прогноз на
...
`
export const ToDoList_about = `Мой первый и еденственный проект на Vue). Выполняя его разбирался с синтаксисом 
Вью, как он устроен. В начале выполнил TO DO LIST, а потом Weather, и как я уже писал ранее на гранях куба, 
мне больше понравился React. Задачи, которые записаны в список дел, сортируются на "Общий список дел", "Важное", "Список не выплненых" 
и "Список выполненых". В этом проекте я использовал localstorage для хранения задач. Так же есть мобильная версия.`
export const Form_about = `Ни самый сложный проекте. Просто выполняя его, я учился работать с 
формой, ее валидацией, регулярными выражениями, обьектом FileReader и немного поигрался с прелоудерами. На фоне
множества других форм выделяется тем, что здесь есть превью выбронного изображения. Стек: JS, CSS, HTML.`
export const Taplink_about = `Не сложные проектик. Taplink это сайтик с сылками на соц сети владельца этого сайта, скидки и купоны,
